...
export class StorageService extends RestService<Storage> {
  public constructor() {
    ...
    this.selectSolve = (req, query) => {
      query.whereClear();
      query.selectRaw('`update`.version_name,`update`.app_id,`update`.version_code,app.name,user1.name as upload_user_name,user2.name as delete_user_name FROM storage ' 
