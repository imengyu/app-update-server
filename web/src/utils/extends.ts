...
dateProtoType.format = function (formatStr : string) {
  ...
  str = str.replace(/MM/, common.pad(this.getMonth() + 1, 2));
  str = str.replace(/M/, this.getMonth() + 1);
  str = str.replace(/dd|DD/, common.pad(this.getDate(), 2));
  str = str.replace(/d/, this.getDate());
  str = str.replace(/HH/, common.pad(this.getHours(), 2));
  str = str.replace(/hh/, common.pad(this.getHours() > 12 ? this.getHours() - 12 : this.getHours(), 2));
  str = str.replace(/mm/, common.pad(this.getMinutes(), 2));
  str = str.replace(/ii/, common.pad(this.getMinutes(), 2));
  str = str.replace(/ss/, common.pad(this.getSeconds(), 2));
  return str;
}
